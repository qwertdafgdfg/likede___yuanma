...
(3) 在售货机服务项目`lkd_vms_service`里添加和ElasticSearch通信的相关引用，及配置中心里添加对ElasticSearch的相关配置
...
前面我们实现了搜索售货机的功能，当用户搜索到并选取了其中一台设备之后，接下来为了完成用户的购物流程，就需要把售货机里的商品列表及商品详情展示出来，以方便用户的下一步的支付购物操作。

### 2.2 实现思路

(1) 用户选取售货机之后，小程序端将售货机编号传入到小程序后台接口
...
(1) 在`service_common`项目中定义前端需要展示的商品详情对象【删除】
...
(2) 在售货机服务项目`lkd_vms_service`中的售货机服务接口`VendingMachineService`中添加根据售货机编号获取商品列表，及根据售货机编号的方法定义【删除】
...
(4) 在小程序后端服务中的`VMController`中添加获取商品列表和商品详情的RESTful接口方法实现
...
(1) 在订单服务中的consul配置中心添加微信支付相关的配置：
...
(2) 在订单服务`lkd_order_service`中添加微信支付的相关依赖，并且定义配置映射类，微信sdk配置`com.github.wxpay.sdk.WXPayConfig`实现类：
